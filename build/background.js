...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  const defaultSettings = {
    ...
    debugMode: false, // DEBUG MODE: Show popup for every paste event
    whitelistedSites: [],
    blacklistedSites: [],
...
chrome.action.onClicked.addListener(async (tab) => {
  try {
    ...
    const currentState = settings.extensionEnabled !== false;
    
    // If extension is disabled, enable it
    if (!currentState) {
      await handleToggleExtension(true, () => {});
      console.log('GuardPasteAI: Extension enabled via icon click');
    }
    
    // Ensure all content scripts are notified of the state change
...
chrome.commands.onCommand.addListener(async (command) => {
  if (command === 'toggle-debug-mode') {
    try {
      ...
      await chrome.storage.sync.set({ debugMode: newDebugMode });
      
      // Show notification
      chrome.notifications.create({
        type: 'basic',
        iconUrl: 'icons/icon128.svg',
        title: 'GuardPasteAI Debug Mode',
        message: `Debug mode ${newDebugMode ? 'ENABLED' : 'DISABLED'}. ${newDebugMode ? 'Every paste will show a popup.' : 'Normal detection mode.'}`
      });
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  switch (message.action) {
    ...
    case 'toggleDebugMode':
      handleToggleDebugMode(sendResponse);
      return true; // Indicates async response
...
// Toggle debug mode
async function handleToggleDebugMode(sendResponse) {
  try {
    const result = await chrome.storage.sync.get(['debugMode']);
    const newDebugMode = !result.debugMode;
